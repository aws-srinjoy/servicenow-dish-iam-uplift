...
#####################################
Rule Name:
  AUTHORIZED_IAM-KMS-DELETE-KEY-MATERIAL
...
Rule Parameters:
  ...
  Scenario: 3
    Given: Rules parameter is provided
      ...
      And: Only the whitelisted Principal ARN(s) is allowed to perform delete imported key material
     Then: Return COMPLIANT
  Scenario: 4
    Given: Rules parameter is provided
      And: Principal ARN is in whitelisted ARNs
      And: Other Principals than the specified Principal ARN are allowed to perform delete imported key material
     Then: Return NON_COMPLIANT with annotation
  Scenario: 5
    Given: Rules parameter is not provided
      And: KMS Key is active
      And: KMS Key does not allow to delete imported key material
     Then: Return COMPLIANT
  Scenario: 6
    Given: Rules parameter is not provided
      And: KMS Key is active
      And: KMS Key allows to delete imported key material
     Then: Return NON_COMPLIANT with annotation
  Scenario: 7
    Given: Rules parameter is not provided
...
# Define the delete imported key material permission
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if configuration_item and \
        ('configuration' in configuration_item) and \
            ('keyId' in configuration_item['configuration']):
        all_kms_key_list = [configuration_item['configuration']['keyId']]
    else:
    ...
    for key_id in all_kms_key_list:
        ...
        if kms_key_details['KeyMetadata']['Origin'] != 'EXTERNAL':
            evaluations.append(build_evaluation(key_id, 'NOT_APPLICABLE', event))
            continue
        ...
        if key_state == 'Enabled':
            evaluations.append(build_evaluation(key_id, compliant_state(
                               len(allowed_principles)), event))
        else:
            evaluations.append(build_evaluation(key_id, compliant_state(
                               len(allowed_principles)), event,
...
def evaluate_parameters(rule_parameters):
    ...
    """
    if rule_parameters:
        if 'WhitelistedPrincipalArns' not in rule_parameters:
